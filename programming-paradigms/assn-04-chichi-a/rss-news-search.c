...
static void BuildIndices(const char *feedsFileName,hashset *stop_words,hashset *indWords,vector *articles);
static void ProcessFeed(const char *remoteDocumentName,hashset *stop_words,hashset *indWords,vector *articles);
static void PullAllNewsItems(FILE *dataStream,hashset *stop_words,hashset *indWords,vector *articles);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st,hashset *stop_words,hashset *indWords,vector *articles);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         hashset *stop_words,hashset *indWords,vector *articles);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL,
                        hashset *stop_words,hashset *indWords,vector *articles);
static void QueryIndices(hashset *stop_words,hashset *indWords,vector *articles);
static void ProcessResponse(const char *word,hashset *stop_words,hashset *indWords,vector *articles);
...
typedef struct {
  ...
  char *title;
  char *serverName;
  int count;  // number of times specific word appeared in the same article
} article;

// this struct helps us save every indexed word with its article 
typedef struct{
  char* word;
  vector articles;
}indWord;
...
static void freeString(void* s);
static void loadStopWords(hashset* stop_words);
static int indWordHash(const void* w,int numBuckets);
static int indWordCompare(const void* a,const void* b);
static void freeIndWord(void* w);
static int articleCompare1(const void *a, const void *b);
static int articleCompare2(const void *a, const void *b);
static void articleFree(void *a);
// helper methods to parse articles
static article* addNewArticle(const char *articleTitle, const char *articleURL,hashset *indWords,vector *articles);
static void add_ind_words(hashset *stop_words,hashset *indWords,vector *articles, char* word, article* art);
...
int main(int argc, char **argv) {
  ...
  hashset stop_words;
  HashSetNew(&stop_words,sizeof(char*),KNumBuckets,StringHash,StringCompare,freeString);
  loadStopWords(&stop_words);

  hashset indWords;
  HashSetNew(&indWords,sizeof(indWord),KNumBuckets,indWordHash,indWordCompare,freeIndWord);
  
  vector articles;
  VectorNew(&articles,sizeof(article),articleFree,200);

  setbuf(stdout, NULL);
  curl_global_init(CURL_GLOBAL_DEFAULT);
  Welcome(kWelcomeTextFile);
  
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1],&stop_words,&indWords,&articles);

  QueryIndices(&stop_words,&indWords,&articles);
  curl_global_cleanup();
  
  // clean up garbage
  VectorDispose(&articles);
  HashSetDispose(&stop_words);
  HashSetDispose(&indWords);
...
static void BuildIndices(const char *feedsFileName,hashset *stop_words,hashset *indWords,vector *articles) {
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName,stop_words,indWords,articles);
  }
 ...
 *  document, from locally */

static void ProcessFeedFromFile(char *fileName,hashset *stop_words,hashset *indWords,vector *articles) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName,stop_words,indWords,articles);
...
static void ProcessFeed(const char *remoteDocumentName,hashset *stop_words,hashset *indWords,vector *articles) {
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix),stop_words,indWords,articles);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed,stop_words,indWords,articles);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream,hashset *stop_words,hashset *indWords,vector *articles) {
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st,stop_words,indWords,articles);
  }
  STDispose(&st);
}
...
static void ProcessSingleNewsItem(streamtokenizer *st,hashset *stop_words,hashset *indWords,vector *articles) {
  ...
  ParseArticle(articleTitle, articleDescription, articleURL,stop_words,indWords,articles);
}
...
static void ParseArticle(const char *articleTitle,
                         const char *articleDescription,
                         const char *articleURL,hashset *stop_words,hashset *indWords,vector *articles) {
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL,stop_words,indWords,articles);
  STDispose(&st);
  fclose(tmpDoc);
}

// used to create new article based on the current url
// after the program ends we dispose of each article 
static article* addNewArticle(const char *articleTitle, const char *articleURL,hashset *indWords,vector *articles){

   article art;
   url curr_url; 
   URLNewAbsolute(&curr_url,articleURL);
  
   art.title = strdup(articleTitle);
   art.serverName = strdup(curr_url.serverName);
   art.url = strdup(articleURL);
   art.count = 1;

   int ind = VectorSearch(articles,&art,articleCompare2,0,false);

   if (ind == -1){
      VectorAppend(articles,&art);
      URLDispose(&curr_url);
      return VectorNth(articles,VectorLength(articles) - 1);
   }
   else{
     articleFree(&art);
     URLDispose(&curr_url);
     return VectorNth(articles,ind);
   }
...
static void add_ind_words(hashset *stop_words,hashset *indWords,vector *articles, char* word, article* art){
    indWord indexed_word;
    indexed_word.word = strdup(word);
    
    indWord* ind_w = HashSetLookup(indWords,&indexed_word);
    article new_art;
    
    new_art.count = 1;
    new_art.title = strdup(art->title);
    new_art.url = strdup(art->url);
    new_art.serverName = strdup(art->serverName);  

    // ------ check if this new indexed words exists or not already:
    if (ind_w == NULL){            
      VectorNew(&indexed_word.articles,sizeof(article),articleFree,8);
      VectorAppend(&indexed_word.articles,&new_art);
      HashSetEnter(indWords,&indexed_word);
    }

    // ------ if the indexed word already existed in our hashset : 
    else{
      freeString(&indexed_word.word);
      int pos = VectorSearch(&ind_w->articles,&new_art,articleCompare2,0,false);

      // if we found a same article again we just make its count biggger
      if (pos != -1){
          articleFree(&new_art);
          article* n_art = VectorNth(&ind_w->articles,pos);
          n_art->count++;
      }
      // if not, we add new article to our words vector
      else{             
          VectorAppend(&ind_w->articles,&new_art);
      }
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL,hashset *stop_words,hashset *indWords,vector *articles) {
  
  // article we are working on currently
  article* art = addNewArticle(articleTitle,articleURL,indWords,articles);
  if (art == NULL) return;
  ...
  while (STNextToken(st, word, sizeof(word))) {    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        ...
        // --------------------- my implemetation starts here : 
        char* w = strdup(word);
        if (HashSetLookup(stop_words,&w) != NULL) {
          free(w);
          continue;
        }
        free(w);
        //add every indexed word with its article to our hashset 
        add_ind_words(stop_words,indWords,articles,word,art);
...
static void QueryIndices(hashset *stop_words,hashset *indWords,vector *articles) {
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response,stop_words,indWords,articles);
  }
}
...
static void ProcessResponse(const char *word,hashset *stop_words,hashset *indWords,vector *articles) {
  if (WordIsWellFormed(word)) {
      indWord curr_w;
      curr_w.word = strdup(word);

      indWord* check = HashSetLookup(stop_words,&curr_w);

      // if we encounter this word in stop words
      if (check != NULL){
          free(curr_w.word);
      ...
      // case when this words doesn't exist in our hasshet 
      indWord* in_w = HashSetLookup(indWords,&curr_w);
      if (in_w == NULL){
         free(curr_w.word);
      ...
      free(curr_w.word);
      // sorting vectors using it's count 
      // ( how many times this word appeared in this article)
      VectorSort(&in_w->articles,articleCompare1);
     
      int len = VectorLength(&in_w->articles);
      int count = (len - 10);
      if (len < 10){
        count = 0;
      }
      int step = 1;
      //generate answer for every article for this word
      for (int i = len - 1; i >=count; i--){
          article* curr_art = VectorNth(&in_w->articles,i);
          if (curr_art->count == 1){
            printf("%d.) \"%s\" [search term occurs %d time]\n", step, curr_art->title,curr_art->count);
          }
          else{
...
// ======================================   my implemented methods below  =======================================================================

// methods to creade char* hashsets : 
static int StringHash(const void *s, int numBuckets)  
{            
  int i;
...
static void loadStopWords(hashset* stop_words){
    ...
    word_file = fopen("data/stop-words.txt", "r");
    assert(word_file != NULL);

    streamtokenizer st;
    STNew(&st, word_file, kNewLineDelimiters, true);
    
    while (STNextToken(&st, word_buffer, sizeof(word_buffer))) {
          char* word = strdup(word_buffer);
          HashSetEnter(stop_words,&word);
    }
...
// methods to create hashset of every indexed word :

static int indWordHash(const void* w,int numBuckets){
  int i;
  unsigned long hashcode = 0;

  indWord inw = *(indWord*) w;
  char* str = inw.word; 
...
static int indWordCompare(const void* a,const void* b){
  ...
  indWord inw1 = *(indWord*) a; 
  char* a_str = inw1.word; 

  indWord inw2 = *(indWord*) b; 
  char* b_str = inw2.word;
...
static void freeIndWord(void* w){
  indWord* inw = (indWord*) w;
  free(inw->word);
  VectorDispose(&inw->articles);
}

static int articleCompare1(const void *a, const void *b){
     article* art1 = (article*) a;
     article* art2 = (article*) b;

     return (art1->count - art2->count);
}
...
static int articleCompare2(const void *a, const void *b){
     ...
     if (strcasecmp(art1->title,art2->title) == 0 && strcasecmp(art1->serverName,art2->serverName)){
        return 0;
     }
