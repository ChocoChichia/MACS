...
(define (partition pivot crcls)
  (if (null? crcls) '(() ())
      (let ((split-of-rest (partition pivot (cdr crcls))))
	(if (< (car crcls) pivot)
	    (list (cons (car crcls) (car split-of-rest)) (cadr split-of-rest))
	    (list (car split-of-rest) (cons (car crcls) (car (cdr split-of-rest))))))))
...
(define (quicksort crcls)
  (if (<= (length crcls) 1) crcls
      (let ((split (partition (car crcls) (cdr crcls))))
...
;; curr is a current circle from the list  and right_circles is a list
;; with all the circles to the right side of the current one

(define (helper curr right_circles)

  (if (null? right_circles) '() 
    ( append  
      (intersect curr (car right_circles) )
      (helper curr (cdr right_circles ) )
    )
  )
)
...
(define (intersection-points circles)
    ...
    (if (null? circles) '()
        ( append
            (helper (car circles) (cdr circles) )
...
(define (eq_points point1 point2)
  (and (= (car point1) (car point2)) 
       (= (cadr point1) (cadr point2))
  )
)

;; returns a lst without point equal to curr_point
(define (distinct curr_point lst) 
    ( if (null? lst) '()
         (if (eq_points curr_point  (car lst))
             ( distinct curr_point (cdr lst) )
             (cons (car lst) (distinct curr_point (cdr lst) ) )  
         )
    )
)
...
(define (distance-product point lst)
  (apply *
      (map  (lambda (elem)  (dist point elem) )  (distinct point lst))
  )
) 
...
(define (partition pivot crcls)
  (if (null? crcls) '(() ())
      (let ((split-of-rest (partition pivot (cdr crcls))))
	(if (< (get_rate (car crcls)) (get_rate pivot))
	    (list (cons (car crcls) (car split-of-rest)) (cadr split-of-rest))
	    (list (car split-of-rest) (cons (car crcls) (car (cdr split-of-rest))))))))

(define (sort-points crcls)
  (if (<= (length crcls) 1) crcls
      (let ((split (partition (car crcls) (cdr crcls))))
...
(define (clump_len lst)
  (quotient  (length lst) 2)
)

(define (get_point lst)

  (cadr lst)

)

(define (clumps rate_lst len)

  (if (or (= len 0) (null? rate_lst)) '()
      (cons (get_point (car rate_lst)) (clumps (cdr rate_lst) (- len 1) ) )
  )
)

(define (clumped-points points)
  (let ((rated (rate-points points)))
    (clumps (sort-points rated) (clump_len rated))
  )
)
...
(define (avgX points len)
  (/  (apply + 
      (map (lambda (elem) (car elem) ) points)
      )
      len
  )
)

(define (avgY points len)
  (/  (apply + 
      (map (lambda (elem) (cadr elem) ) points)
      )
      len
  )
)
...
(define (average-point points) 
  ( let* ( (av_x (avgX points (length points) ) )  (av_y (avgY points (length points) ) ) 
        (pnt ( list av_x av_y ) ) )
    (list (distance-product pnt points)  pnt )
  )
)
